...
2. Back up database
3. `git fetch` and `git pull` to update the codebase on the server
4. Update `.env` as necessary from the update
5. Run `pip3 install -r requirements.txt`
6. Run `manage.py migrate` from the project directory
7. Run `manage.py collectstatic` from the project directory (is this a required step?)
